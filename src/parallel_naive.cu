...
__global__ void scatterPR(
    ...
    if (u < n) {
        ...
        if (start_edge < end_edge) {
            float contribution = pr_in[u] / (float)(end_edge - start_edge);

            for (int i = start_edge; i < end_edge; ++i) {
                int v = col_indices[i];
                atomicAdd(&pr_out[v], contribution);
            }
        }
    }
}

__global__ void damping(
    const float *pr_out,
    float *pr_in,
    float dangling_sum,
    int n
    ...
    int u = blockIdx.x * blockDim.x + threadIdx.x;
    if (u < n) {
        pr_in[u] = (1.0f - DELTA) / n + DELTA * (dangling_sum / n + pr_out[u]);
    }
}
...
int main(int argc, char **argv) {
    ...
    h_row_ptr[0] = 0;
    for (int u = 0; u < n; ++u) {
        int in_deg = adj[u].size();
        memcpy(h_col_indices + h_row_ptr[u], adj[u].data(), in_deg * sizeof(int));
    ...
    initPR<<<grid_size, BLOCK_SIZE>>>(d_pr_in, n);
    cudaCheckError(cudaPeekAtLastError());

    int iter = 0;
    for (; iter < MAX_ITER; ++iter) {
        cudaCheckError(cudaMemset(d_pr_out, 0, n * sizeof(float)));
        scatterPR<<<grid_size, BLOCK_SIZE>>>(d_col_indices, d_row_ptr, d_pr_in, d_pr_out, n);

        cudaCheckError(cudaMemset(d_dangling_sum, 0, sizeof(float)));
        danglingSum<<<REDUCE_GRID_SIZE, BLOCK_SIZE>>>(d_row_ptr, d_pr_in, d_dangling_sum, n);
        float h_dangling_sum;
        cudaMemcpy(&h_dangling_sum, d_dangling_sum, sizeof(float), cudaMemcpyDeviceToHost);

        damping<<<grid_size, BLOCK_SIZE>>>(d_pr_out, d_pr_in, h_dangling_sum, n);
    }
